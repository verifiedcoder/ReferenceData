...
using (var context = sp.GetRequiredService<ReferenceDataContext>())
{
    ...
    var availableColors = context.AvailableColors.AsNoTracking().ToList();

    // List the reference data entities retrieved from the database.
    availableColors.ForEach(c => Console.WriteLine($"ID: '{c.Id}', Description: '{c.Description}'."));
    ...
    availableColors.Select(ReferenceDataConverter.ConvertReferenceDataEntityToEnum<AvailableColorEntity, AvailableColor>)
           .ToList()
